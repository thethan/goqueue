...
func Test_LRange_GetItems_Integrations(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		lrange := NewLRangeQueue("queue:default", redisClient)
		...
		for j := range jobChan {
			...
			klass, err := j.GetValue("class")
			errMessage, err := j.GetValue("error_message")

			if errMessage.Kind() != reflect.Invalid && klass.String() == "WebhookWorker" {
				fmt.Println(klass.String(), errMessage.String(), val.Slice(0, 1), err)
			}
		}
	})
}
